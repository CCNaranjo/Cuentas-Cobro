...
<aside class="w-64 bg-primary shadow-xl flex-shrink-0 hidden lg:flex flex-col">
    ...
    <nav class="flex-1 overflow-y-auto mt-6 px-3">
        ...
        <div class="mb-6">
            ...
            @if(auth()->user()->tienePermiso('ver-usuarios', session('organizacion_actual')))
            <a href="{{ route('usuarios.index') }}" 
               class="sidebar-link group {{ request()->routeIs('usuarios.*') ? 'active' : '' }}">
                ...
                @endphp
                @if($pendientes > 0)
                    <span class="ml-auto bg-warning text-white px-2 py-0.5 rounded-full text-xs font-bold">{{ $pendientes }}</span>
                @endif
            </a>
            @endif
